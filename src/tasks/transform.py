...
def remove_modfications_duplicates(df):
    ...
    df_cleaned = df.with_columns(
        short_id=pl.col("uid").str.head(-2),
    ...
    df_cleaned = df_cleaned.with_columns(
        uid=pl.when(pl.col("modif_id").is_duplicated())
...
def replace_by_modification_data(df):
    ...
    df = df.with_columns(
        pl.col("modification").struct.rename_fields(
            [f"modification.{field}" for field in modification_columns]
        )
    ).unnest("modification")
    # On conserve la dernière valeur non_null pour tous les champs de modification
    df = df.with_columns(
        **{
            f"last_{_col}": pl.col(f"modification.{_col}")
            .forward_fill()
            .over("modif_id")
            for _col in modification_columns
        }
    )
    df = df.drop([f"modification.{field}" for field in modification_columns])
    ...
    for _col in modification_columns:
        if not isinstance(_schema[f"last_{_col}"], pl.List):
            df = df.with_columns(
                **{
                    _col: pl.when(pl.col(f"last_{_col}").is_not_null())
                    .then(pl.col(f"last_{_col}"))
            ...
            print(
                f"{_col} a un type imprévu:",
                df.filter(pl.col(f"last_{_col}").is_not_null()).select(f"last_{_col}"),
            )
        pass
    df = df.drop([f"last_{_col}" for _col in modification_columns])
